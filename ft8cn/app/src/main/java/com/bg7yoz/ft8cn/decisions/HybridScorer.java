 ...
 *
 * Score = Σ(weight[c] * dynamic_factor[c] * value[c]) for all criteria c
 *
 * @author BG7YOZ
 * @date 2026-05-10
 */
public class HybridScorer {
    ...
    public float score(DatabaseOpr.StationRecord s, DecisionContext ctx) {
        ...
        for (Criterion c : Criterion.values()) {
            ...
            float weight = ctx.weights[c.ordinal()]; // [FIX] Было: ctx.criterionWeights
    ...
    private float getValueForCriterion(Criterion c, DatabaseOpr.StationRecord s, DecisionContext ctx) {
        switch (c) {
            ...
            case RARITY:
                ...
                int currentBandBit = DatabaseOpr.freqToBandBit(com.bg7yoz.ft8cn.GeneralVariables.band);
    ...
    private float getDynamicFactor(Criterion c, DatabaseOpr.StationRecord s, DecisionContext ctx) {
        switch (c) {
            ...
            case RARITY:
                // [FIX] Было: ctx.dxModeEnabled → ctx.acceptDxCalls
